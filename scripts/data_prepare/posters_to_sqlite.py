...
def store_poster(db_path: str, movie_id: int, movie_name: str, poster_path: Path) -> bool:
    ...
    try:
        ...
        c.execute('INSERT OR REPLACE INTO movie_posters (movie_id, movie_name, poster_data) VALUES (?, ?, ?)',
...
def process_posters(posters_dir: str, db_path: str) -> None:
    ...
    for poster_path in tqdm(poster_files, desc="Storing posters"):
        ...
        movie_name = '_'.join(filename.split('_')[1:])  # Join remaining parts as movie name
...
if __name__ == "__main__":
    ...
    c.execute('SELECT movie_id, movie_name FROM movie_posters ORDER BY RANDOM() LIMIT 1')
